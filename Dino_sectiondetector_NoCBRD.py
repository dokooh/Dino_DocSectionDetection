...
class PDFSectionDetector:
    ...
    # ============================================================================
    # COMMENTED OUT: Color-Based Region Detection (CBRD)
    # ============================================================================
    # The following method implements color-based region detection using OpenCV
    # to identify non-white (colored) regions in PDF pages. This approach relies
    # on morphological operations and contour detection to find sections.
    # ============================================================================
    ...
    # ============================================================================
    # ALTERNATIVE: Manual/Grid-based Region Detection
    # ============================================================================
    # This replaces the color-based detection with alternative approaches:
    # 1. Manual bounding box specification
    # 2. Grid-based uniform sampling
    # 3. Full-page analysis
    # ============================================================================
    
    def detect_regions_manual(self, image: Image.Image, 
                             manual_bboxes: List[Tuple[int, int, int, int]] = None,
                             min_width: int = 50,
                             min_height: int = 20) -> List[Tuple[int, int, int, int]]:
        """
        Manual region detection - specify bounding boxes directly
        
        Args:
            image: PIL Image
            manual_bboxes: List of manually specified bounding boxes (x1, y1, x2, y2)
            min_width: Minimum width for bounding boxes in pixels
            min_height: Minimum height for bounding boxes in pixels
            
        Returns:
            List of bounding boxes (x1, y1, x2, y2) for specified regions
        """
        if manual_bboxes is None:
            print("  No manual bounding boxes specified - using default regions")
            # Default to some standard regions (header, body, footer)
            width, height = image.size
            manual_bboxes = [
                (50, 50, width - 50, 150),           # Header region
                (50, 200, width - 50, height - 200), # Body region
                (50, height - 150, width - 50, height - 50) # Footer region
            ]
        
        print(f"  Using {len(manual_bboxes)} manually specified regions")
        
        # Filter small bounding boxes
        filtered_bboxes = self._filter_small_bboxes(manual_bboxes, min_width=min_width, min_height=min_height)
        
        return filtered_bboxes
    
    def detect_regions_grid(self, image: Image.Image,
                           grid_rows: int = 3, grid_cols: int = 2,
                           overlap_ratio: float = 0.1,
                           min_width: int = 50,
                           min_height: int = 20) -> List[Tuple[int, int, int, int]]:
        """
        Grid-based region detection - divide page into uniform grid
        
        Args:
            image: PIL Image
            grid_rows: Number of rows in the grid
            grid_cols: Number of columns in the grid
            overlap_ratio: Overlap between adjacent grid cells (0.0 to 0.5)
            min_width: Minimum width for bounding boxes in pixels
            min_height: Minimum height for bounding boxes in pixels
            
        Returns:
            List of bounding boxes (x1, y1, x2, y2) for grid regions
        """
        width, height = image.size
        ...
        overlap_x = int(cell_width * overlap_ratio)
        overlap_y = int(cell_height * overlap_ratio)
        ...
        for row in range(grid_rows):
            for col in range(grid_cols):
                ...
                x2 = min(width, (col + 1) * cell_width + overlap_x)
                y2 = min(height, (row + 1) * cell_height + overlap_y)
                
                bbox = (x1, y1, x2, y2)
                bboxes.append(bbox)
        
        print(f"  Generated {len(bboxes)} grid-based regions ({grid_rows}x{grid_cols})")
    ...
    def detect_regions_full_page(self, image: Image.Image,
                                min_width: int = 50,
                                min_height: int = 20) -> List[Tuple[int, int, int, int]]:
        """
        Full-page region detection - treat entire page as one region
        ...
        Returns:
            List containing single bounding box for entire page
        """
        width, height = image.size
        ...
        print(f"  Using full page as single region: {bbox}")
        
        # Filter small bounding boxes (though full page should never be small)
        filtered_bboxes = self._filter_small_bboxes([bbox], min_width=min_width, min_height=min_height)
    ...
    def process_page(self, image: Image.Image, page_num: int, 
                     detection_method: str = 'grid',
                     manual_bboxes: List[Tuple[int, int, int, int]] = None,
                     grid_rows: int = 3, grid_cols: int = 2,
                     min_width: int = 50, min_height: int = 20) -> List[Section]:
        """
        Process a single page and detect sections (WITHOUT Color-Based Region Detection)
        
        Args:
            image: PIL Image
            page_num: Page number
            detection_method: Method to use ('manual', 'grid', 'full_page')
            manual_bboxes: Manual bounding boxes (for 'manual' method)
            grid_rows: Number of grid rows (for 'grid' method)
            grid_cols: Number of grid columns (for 'grid' method)
            min_width: Minimum width for bounding boxes in pixels
            min_height: Minimum height for bounding boxes in pixels
        ...
        """
        print(f"Processing page {page_num} (method: {detection_method})...")
        
        # REPLACED: Color-based region detection with alternative methods
        if detection_method == 'manual':
            bboxes = self.detect_regions_manual(image, manual_bboxes, min_width, min_height)
        elif detection_method == 'grid':
            bboxes = self.detect_regions_grid(image, grid_rows, grid_cols, 0.1, min_width, min_height)
        elif detection_method == 'full_page':
            bboxes = self.detect_regions_full_page(image, min_width, min_height)
        else:
            raise ValueError(f"Unknown detection method: {detection_method}")
        
        print(f"  Found {len(bboxes)} regions using {detection_method} method")
    ...
    def process_pdf(self, pdf_path: str, output_json: str = None, 
                    dpi: int = 300, detection_method: str = 'grid',
                    manual_bboxes: List[Tuple[int, int, int, int]] = None,
                    grid_rows: int = 3, grid_cols: int = 2,
                    eps: float = 0.5, enable_clustering: bool = True) -> List[Section]:
        """
        Process entire PDF document (WITHOUT Color-Based Region Detection)
        
        Args:
            pdf_path: Path to PDF file
            output_json: Optional path to save results as JSON
            dpi: DPI for PDF to image conversion
            detection_method: Region detection method ('manual', 'grid', 'full_page')
            manual_bboxes: Manual bounding boxes (for 'manual' method)
            grid_rows: Number of grid rows (for 'grid' method)
            grid_cols: Number of grid columns (for 'grid' method)
        ...
        for page_num, image in enumerate(images, start=1):
            sections = self.process_page(
                image, page_num, detection_method, manual_bboxes, 
                grid_rows, grid_cols, min_width=50, min_height=20
            )
        ...
        if output_json:
            data = {
                'metadata': {
                    ...
                    'grid_rows': int(grid_rows) if detection_method == 'grid' else None,
                    'grid_cols': int(grid_cols) if detection_method == 'grid' else None,
                    'clustering_eps': float(eps),
                    'clustering_enabled': enable_clustering,
                    'total_pages': len(images),
                    'total_sections': len(all_sections),
                    'color_based_detection': False  # Explicitly mark as disabled
                },
                'sections': [
                    {
                        'page': int(s.page_num),
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="PDF Section Detection using DINO embeddings (NO Color-Based Region Detection)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python Dino_sectiondetector_NoCBRD.py document.pdf
  python Dino_sectiondetector_NoCBRD.py document.pdf -o results.json
  python Dino_sectiondetector_NoCBRD.py document.pdf -m facebook/dinov2-large --dpi 150
  python Dino_sectiondetector_NoCBRD.py document.pdf --detection-method grid --grid-rows 4 --grid-cols 3
  python Dino_sectiondetector_NoCBRD.py document.pdf --detection-method full_page
  python Dino_sectiondetector_NoCBRD.py document.pdf --output-images ./annotated/ --verbose
        """
    )
    ...
    parser.add_argument(
        '-o', '--output',
        default=None,
        help='Output JSON file path (default: input_filename_sections_NoCBRD.json)'
    )
    ...
    parser.add_argument(
        '--detection-method',
        choices=['manual', 'grid', 'full_page'],
        default='grid',
        help='Region detection method (default: grid)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help='Number of grid rows for grid method (default: 3)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help='Number of grid columns for grid method (default: 2)'
    )
...
def validate_arguments(args):
    ...
    if args.output is None:
        ...
        args.output = f"{base_name}_sections_NoCBRD.json"
    ...
    if args.grid_rows < 1 or args.grid_cols < 1:
        print("Error: Grid rows and columns must be at least 1.")
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    # Print configuration
    print("PDF Section Detection Configuration (NO Color-Based Region Detection):")
    print(f"  Input PDF: {args.input_pdf}")
    print(f"  Output JSON: {args.output}")
    print(f"  Model: {args.model}")
    print(f"  DPI: {args.dpi}")
    print(f"  Detection method: {args.detection_method}")
    if args.detection_method == 'grid':
    ...
    print(f"  Color-Based Region Detection: DISABLED")
    print("-" * 50)
    ...
    try:
        ...
        for page_num, image in enumerate(images, start=1):
            ...
            # Process page with selected detection method
            sections = detector.process_page(
                image, page_num, 
                detection_method=args.detection_method,
                manual_bboxes=None,  # Could be extended to support manual bbox specification
        ...
        if args.output_images:
            ...
            for page_num, image in enumerate(images, start=1):
                ...
                if page_sections:
                    ...
                    output_path = os.path.join(args.output_images, f"annotated_page_{page_num:03d}_NoCBRD.png")
        ...
        data = {
            'metadata': {
                ...
                'dpi': int(args.dpi),
                'detection_method': args.detection_method,
                'grid_rows': int(args.grid_rows) if args.detection_method == 'grid' else None,
                'grid_cols': int(args.grid_cols) if args.detection_method == 'grid' else None,
        ...
        print(f"\nNote: Color-Based Region Detection was DISABLED in this run.")
        print(f"Used detection method: {args.detection_method}")
